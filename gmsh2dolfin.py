...
def help():
    print("Usage: {} [OPTION..] MSHFILE XDMFSURFFILE XDMFVOLFILE"
          .format(program_name))
    print("")
...
def main(args):
    ...
    if len(args) < 1:
        print("{}: please specify a Gmsh MSH file"
    ...
    if len(args) < 2:
        print("{}: please specify a DOLFIN XDMF output file for writing the mesh surface"
              .format(program_invocation_short_name), file=sys.stderr)
        sys.exit(1)
    xdmfsurfpath = args[1]
    h5surfpath = os.path.splitext(xdmfsurfpath)[0] + '.h5'

    if len(args) < 3:
        print("{}: please specify a DOLFIN XDMF output file for writing the mesh volume"
              .format(program_invocation_short_name), file=sys.stderr)
        sys.exit(1)
    xdmfvolpath = args[2]
    ...
    if len(args) > 3:
        print("{}: Invalid argument {}"
              .format(program_invocation_short_name, args[3]),
              file=sys.stderr)
        sys.exit(1)
    ...
    if verbose:
        t1 = time.time()
        size = None
        try: size = os.path.getsize(mshpath)
        except: pass
        if size is not None:
