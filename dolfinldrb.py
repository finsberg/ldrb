...
def help():
    print(
        "Usage: {} [OPTION..] XDMFSURFFILE XDMFVOLFILE FIBREFILE SHEETFILE NORMALFILE".format(
            program_name,
        ),
    )
    print("")
    ...
    print(
        "  --left-endo-alpha=DEG\t\tleft ventricle endocardium fibre angle (default: 30)",
    )
    print(
        "  --left-epi-alpha=DEG\t\tleft ventricle epicardium fibre angle (default: -30)",
    )
    print(
        "  --left-endo-beta=DEG\t\tleft ventricle endocardium sheet angle (default: 0)",
    )
    print("  --left-epi-beta=DEG\t\tleft ventricle epicardium sheet angle (default: 0)")
    print(
        "  --right-endo-alpha=DEG\tright ventricle endocardium fibre angle (default: 80)",
    )
    print(
        "  --right-epi-alpha=DEG\t\tright ventricle epicardium fibre angle (default: -80)",
    )
    print(
        "  --right-endo-beta=DEG\t\tright ventricle endocardium sheet angle (default: 0)",
    )
    print(
        "  --right-epi-beta=DEG\t\tright ventricle epicardium sheet angle (default: 0)",
    )
    ...
    print(
        "  --krylov-solver\t\tUse a Krylov solver to solve the linear equation systems.",
    )
    ...
    print(
        "  --krylov-solver-atol=TOL\tconvergence tolerance for the residual (default: 1e-15)",
    )
    print(
        "  --krylov-solver-rtol=TOL\tconvergence tolerance for the relative residual (default: 1e-10)",
    )
...
def main(args):
    ...
    while i < len(args):
        ...
        try:
            ...
            print(
                "{}: Invalid argument {}".format(
                    program_invocation_short_name, args[i],
                ),
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        try:
            ...
            if args[i].startswith("--krylov-solver-max-its="):
                krylov_solver_max_its = float(
                    args[i].split("--krylov-solver-max-its=")[1],
                )
                i = i + 1
                continue
        ...
        print(
            "{}: Invalid argument {}".format(program_invocation_short_name, args[i]),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if xdmfsurfpath is None:
        print(
            "{}: please specify a DOLFIN XDMF for "
            "the surface mesh".format(program_invocation_short_name),
            file=sys.stderr,
        )
        sys.exit(1)
    elif xdmfvolpath is None:
        print(
            "{}: please specify a DOLFIN XDMF for "
            "the volume mesh".format(program_invocation_short_name),
            file=sys.stderr,
        )
        sys.exit(1)
    elif fibrepath is None:
        print(
            "{}: please specify a DOLFIN XDMF output file "
            "for writing fibre directions".format(program_invocation_short_name),
            file=sys.stderr,
        )
        sys.exit(1)
    elif sheetpath is None:
        print(
            "{}: please specify a DOLFIN XDMF output file "
            "for writing sheet directions".format(program_invocation_short_name),
            file=sys.stderr,
        )
        sys.exit(1)
    elif normalpath is None:
        print(
            "{}: please specify a DOLFIN XDMF output file "
            "for writing sheet normal directions".format(program_invocation_short_name),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "{}: ldrb not found".format(program_invocation_short_name), file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "{}: dolfin not found".format(program_invocation_short_name),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "{}: warning: mpi4py not found".format(program_invocation_short_name),
            file=sys.stderr,
        )
    ...
    if verbose and rank == 0:
        ...
        size = None
        try:
            size = os.path.getsize(h5fibrepath)
        except:
            pass
        if size is not None:
            print(
    ...
    if verbose and rank == 0:
        ...
        size = None
        try:
            size = os.path.getsize(h5sheetpath)
        except:
            pass
        if size is not None:
            print(
    ...
    if verbose and rank == 0:
        ...
        size = None
        try:
            size = os.path.getsize(h5normalpath)
        except:
            pass
        if size is not None:
            print(
